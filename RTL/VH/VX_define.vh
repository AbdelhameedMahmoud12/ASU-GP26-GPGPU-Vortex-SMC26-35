...
`define REDUCE_TREE(__op, __out, __in, __n, __outw, __inw) \
    VX_reduce_tree #( \
        ...
        .N     (__n), \
        .OP    ("__op") \
    ) reduce_``__out``_``__in ( \
        .data_in(__in), \
        .data_out(__out) \
    )

`define POP_COUNT_EX(tag, out, in, model) \
    VX_popcount #( \
        .N ($bits(in)), \
        .MODEL (model) \
    ) __pop_countex``tag ( \
...
`define ASSIGN_BLOCKED_WID(dst, src, block_idx, block_size) \
    ...
    if (block_size != 1) begin \
        if (block_size != `NUM_WARPS) begin \
            assign dst = {src[NW_WIDTH-1:`CLOG2(block_size)], \
                          block_idx[`CLOG2(block_size)-1:0]}; \
        end else begin \
            assign dst = block_idx[NW_WIDTH-1:0]; \
        end \
    end else begin \
        assign dst = src; \
    end \

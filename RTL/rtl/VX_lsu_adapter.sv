...
module VX_lsu_adapter import VX_gpu_pkg::*; #(
    ...
    // mem_bus request outputs
    output wire [NUM_LANES-1:0]                                         mem_bus_req_valid,
    output wire [NUM_LANES-1:0]                                         mem_bus_req_rw,
    output wire [NUM_LANES-1:0][`MEM_ADDR_WIDTH-`CLOG2(DATA_SIZE)-1:0]  mem_bus_req_addr,
    output wire [NUM_LANES-1:0][DATA_SIZE*8-1:0]                        mem_bus_req_data,
    output wire [NUM_LANES-1:0][DATA_SIZE-1:0]                          mem_bus_req_byteen,
    output wire [NUM_LANES-1:0][MEM_FLAGS_WIDTH-1:0]                    mem_bus_req_flags,
    output wire [NUM_LANES-1:0][TAG_WIDTH-1:0]                          mem_bus_req_tag,
    input  wire [NUM_LANES-1:0]                                         mem_bus_req_ready,

    // mem_bus response inputs
    input  wire [NUM_LANES-1:0]                                         mem_bus_rsp_valid,
    input  wire [NUM_LANES-1:0][DATA_SIZE*8-1:0]                        mem_bus_rsp_data,
    input  wire [NUM_LANES-1:0][TAG_WIDTH-1:0]                          mem_bus_rsp_tag,
    output wire [NUM_LANES-1:0]                                         mem_bus_rsp_ready
);
    ...
    for (genvar i = 0; i < NUM_LANES; ++i) begin : g_mem_bus_req
        assign mem_bus_req_valid[i] = req_valid_out[i];
        assign {
            mem_bus_req_rw[i],
            mem_bus_req_addr[i],
            mem_bus_req_data[i],
            mem_bus_req_byteen[i],
            mem_bus_req_flags[i]
         } = req_data_out[i];
        assign mem_bus_req_tag[i] = req_tag_out[i];
        assign req_ready_out[i] = mem_bus_req_ready[i];
    end
    ...
    for (genvar i = 0; i < NUM_LANES; ++i) begin : g_mem_bus_rsp
        assign rsp_valid_out[i] = mem_bus_rsp_valid[i];
        assign rsp_data_out[i]  = mem_bus_rsp_data[i];
        assign rsp_tag_out[i]   = mem_bus_rsp_tag[i];
        assign mem_bus_rsp_ready[i] = rsp_ready_out[i];
    end

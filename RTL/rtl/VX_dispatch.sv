...
`ifdef PERF_ENABLE
    output wire [PERF_CTR_BITS-1:0] perf_stalls [NUM_EX_UNITS-1:0],
`endif
    // inputs
    VX_operands_if.slave    operands_if,

    // outputs
    output wire [NUM_EX_UNITS-1:0] dispatch_if_valid,
    output wire [NUM_EX_UNITS-1:0][$bits(dispatch_t)-1:0] dispatch_if_data,
    input  wire [NUM_EX_UNITS-1:0] dispatch_if_ready
);
    ...
    assign operands_if.ready = operands_ready_in[(operands_if.data.ex_type < NUM_EX_UNITS) ? operands_if.data.ex_type : 0];

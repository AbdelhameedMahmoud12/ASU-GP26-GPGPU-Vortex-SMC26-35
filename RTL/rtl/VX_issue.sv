...
`endif
    ...
    VX_writeback_if.slave   writeback_if [`ISSUE_WIDTH-1:0],
    VX_dispatch_if.master   dispatch_if [0:(NUM_EX_UNITS * `ISSUE_WIDTH)-1], //If error change to normal and solve any dependency like "for loop in the end of code"
    VX_issue_sched_if.master issue_sched_if[`ISSUE_WIDTH-1:0]
);
...
`endif
    ...
    for (genvar issue_id = 0; issue_id < `ISSUE_WIDTH; ++issue_id) begin : g_slices
        ...
        wire [NUM_EX_UNITS-1:0] per_issue_dispatch_if_valid;
        wire [NUM_EX_UNITS-1:0][$bits(dispatch_t)-1:0] per_issue_dispatch_if_data;
        wire [NUM_EX_UNITS-1:0] per_issue_dispatch_if_ready;
        ...
        `endif
            ...
            .dispatch_if_valid (per_issue_dispatch_if_valid),
            .dispatch_if_data  (per_issue_dispatch_if_data),
            .dispatch_if_ready (per_issue_dispatch_if_ready),
        ...
        for (genvar ex_id = 0; ex_id < NUM_EX_UNITS; ++ex_id) begin : g_dispatch_if
            assign dispatch_if[ex_id * `ISSUE_WIDTH + issue_id].valid = per_issue_dispatch_if_valid[ex_id];
            assign dispatch_if[ex_id * `ISSUE_WIDTH + issue_id].data  = per_issue_dispatch_if_data[ex_id];
            assign per_issue_dispatch_if_ready[ex_id] = dispatch_if[ex_id * `ISSUE_WIDTH + issue_id].ready;
        end
     end

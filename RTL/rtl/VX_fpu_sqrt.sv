...
`ifdef ASIC
    ...
    // ASIC path: pure-RTL FP32 square root — see VX_fsqrt_unit.sv
    for (genvar i = 0; i < NUM_PES; ++i) begin : g_fsqrts
        wire [31:0] fsqrt_result_w;
        wire [`FP_FLAGS_BITS-1:0] fsqrt_fflags_w;
        ...
        VX_fsqrt_unit #(
            ...
            .a        (pe_data_in[i][0 +: 32]),
            .result   (fsqrt_result_w),
            .fflags   (fsqrt_fflags_w)
        );

        assign pe_data_out[i][0  +: 32]             = fsqrt_result_w;
        assign pe_data_out[i][32 +: `FP_FLAGS_BITS] = fsqrt_fflags_w;
    end

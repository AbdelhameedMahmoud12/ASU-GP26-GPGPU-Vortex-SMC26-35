...
`ifdef ASIC
    ...
    // ASIC path: pure-RTL FP32 divider — see VX_fdiv_unit.sv
    for (genvar i = 0; i < NUM_PES; ++i) begin : g_fdivs
        wire [31:0] fdiv_result_w;
        wire [`FP_FLAGS_BITS-1:0] fdiv_fflags_w;
        ...
        VX_fdiv_unit #(
            ...
            .result   (fdiv_result_w),
            .fflags   (fdiv_fflags_w)
        );

        assign pe_data_out[i][0  +: 32]             = fdiv_result_w;
        assign pe_data_out[i][32 +: `FP_FLAGS_BITS] = fdiv_fflags_w;
    end

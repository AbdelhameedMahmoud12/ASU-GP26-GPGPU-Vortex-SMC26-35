...
module VX_pe_switch_alu import VX_gpu_pkg::*; #(
    ...
    output wire [PE_COUNT-1:0]                  execute_out_valid,
    output alu_exe_t [PE_COUNT-1:0]             execute_out_data,
    input  wire [PE_COUNT-1:0]                  execute_out_ready,
    input  wire [PE_COUNT-1:0]                  result_in_valid,
    input  alu_res_t [PE_COUNT-1:0]             result_in_data,
    output wire [PE_COUNT-1:0]                  result_in_ready
);
    ...
    for (genvar i = 0; i < PE_COUNT; ++i) begin : g_execute_out_if
        assign execute_out_valid[i] = pe_req_valid[i];
        assign execute_out_data[i] = pe_req_data[i];
        assign pe_req_ready[i] = execute_out_ready[i];
    end
    ///////////////////////////////////////////////////////////////////////////
    ...
    for (genvar i = 0; i < PE_COUNT; ++i) begin : g_result_in_if
        assign pe_rsp_valid[i] = result_in_valid[i];
        assign pe_rsp_data[i] = result_in_data[i];
        assign result_in_ready[i] = pe_rsp_ready[i];
    end
    VX_stream_arb #(
        .NUM_INPUTS (PE_COUNT),
        .DATAW      (RSP_DATAW),

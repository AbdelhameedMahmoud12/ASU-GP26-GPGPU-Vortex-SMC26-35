...
module VX_alu_muldiv import VX_gpu_pkg::*; #(
    ...
    input alu_exe_t         execute_data,
    output wire             execute_ready,

    // Outputs
    output wire             result_valid,
    output alu_res_t        result_data,
    ...
    `UNUSED_VAR (execute_data.rs3_data)

    wire [INST_M_BITS-1:0] muldiv_op = INST_M_BITS'(execute_data.op_type);

    wire is_mulx_op = inst_m_is_mulx(muldiv_op);
    wire is_signed_op = inst_m_signed(muldiv_op);
`ifdef XLEN_64
    wire is_alu_w = execute_data.op_args.alu.is_w;
`else
    wire is_alu_w = 0;
`endif
    ...
    wire mul_valid_in = execute_valid && is_mulx_op;
    wire mul_ready_in;
    wire mul_valid_out;
    wire mul_ready_out;
...
`ifdef IMUL_DPI
    ...
    for (genvar i = 0; i < NUM_LANES; ++i) begin : g_mul_result_tmp
        ...
        wire [`XLEN-1:0] mul_in1 = is_alu_w ? (execute_data.rs1_data[i] & `XLEN'hFFFFFFFF) : execute_data.rs1_data[i];
        wire [`XLEN-1:0] mul_in2 = is_alu_w ? (execute_data.rs2_data[i] & `XLEN'hFFFFFFFF) : execute_data.rs2_data[i];
        always @(*) begin
    ...
    VX_shift_register #(
        ...
        .reset    (reset),
        .enable   (mul_ready_in),
        .data_in  ({mul_valid_in,  execute_data.uuid, execute_data.wid, execute_data.tmask, execute_data.PC, execute_data.rd, execute_data.wb, execute_data.pid, execute_data.sop, execute_data.eop, mul_result_tmp}),
...
`ifdef XLEN_64
    ...
    for (genvar i = 0; i < NUM_LANES; ++i) begin : g_mul_in
        assign mul_in1[i] = is_alu_w ? {{(`XLEN-31){execute_data.rs1_data[i][31]}}, execute_data.rs1_data[i][31:0]} : {is_signed_mul_a && execute_data.rs1_data[i][`XLEN-1], execute_data.rs1_data[i]};
        assign mul_in2[i] = is_alu_w ? {{(`XLEN-31){execute_data.rs2_data[i][31]}}, execute_data.rs2_data[i][31:0]} : {is_signed_mul_b && execute_data.rs2_data[i][`XLEN-1], execute_data.rs2_data[i]};
    end
    ...
    always @(posedge clk) begin
        if (mul_valid_in && mul_ready_in) begin
            mul_tag_r <= {execute_data.uuid, execute_data.wid, execute_data.tmask, execute_data.PC, execute_data.rd, execute_data.wb, is_mulh_in, is_alu_w, execute_data.pid, execute_data.sop, execute_data.eop};
        end
    end
...
`else
    ...
    for (genvar i = 0; i < NUM_LANES; ++i) begin : g_multiplier
        wire [`XLEN:0] mul_in1 = {is_signed_mul_a && execute_data.rs1_data[i][`XLEN-1], execute_data.rs1_data[i]};
        wire [`XLEN:0] mul_in2 = {is_signed_mul_b && execute_data.rs2_data[i][`XLEN-1], execute_data.rs2_data[i]};
    ...
    VX_shift_register #(
        ...
        .reset    (reset),
        .enable   (mul_ready_in),
        .data_in  ({mul_valid_in,  execute_data.uuid, execute_data.wid, execute_data.tmask, execute_data.PC, execute_data.rd, execute_data.wb, execute_data.pid, execute_data.sop, execute_data.eop, is_mulh_in,  is_alu_w}),
...
`endif
    ...
    wire div_valid_in = execute_valid && ~is_mulx_op;
    wire div_ready_in;
    wire div_valid_out;
    wire div_ready_out;
    ...
    `ifdef XLEN_64
        assign div_in1[i] = is_alu_w ? {{(`XLEN-32){is_signed_op && execute_data.rs1_data[i][31]}}, execute_data.rs1_data[i][31:0]}: execute_data.rs1_data[i];
        assign div_in2[i] = is_alu_w ? {{(`XLEN-32){is_signed_op && execute_data.rs2_data[i][31]}}, execute_data.rs2_data[i][31:0]}: execute_data.rs2_data[i];
    `else
        assign div_in1[i] = execute_data.rs1_data[i];
        assign div_in2[i] = execute_data.rs2_data[i];
    `endif
    end
...
`ifdef IDIV_DPI
    ...
    VX_shift_register #(
        ...
        .reset    (reset),
        .enable   (div_ready_in),
        .data_in  ({div_valid_in,  execute_data.uuid, execute_data.wid, execute_data.tmask, execute_data.PC, execute_data.rd, execute_data.wb, execute_data.pid, execute_data.sop, execute_data.eop, div_result_in}),
...
`else
    ...
    always @(posedge clk) begin
        if (div_valid_in && div_ready_in) begin
            div_tag_r <= {execute_data.uuid, execute_data.wid, execute_data.tmask, execute_data.PC, execute_data.rd, execute_data.wb, is_rem_op, is_alu_w, execute_data.pid, execute_data.sop, execute_data.eop};
        end
    end
...
`endif
    ...
    assign execute_ready = is_mulx_op ? mul_ready_in : div_ready_in;
    ...
    VX_stream_arb #(
        ...
        .data_out  ({result_data.uuid, result_data.wid, result_data.tmask, result_data.PC, result_data.rd, result_data.wb, result_data.pid, result_data.sop, result_data.eop, result_data.data}),

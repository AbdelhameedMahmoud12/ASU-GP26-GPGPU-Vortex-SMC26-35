...
`endif
    ...
    wire [INST_ALU_BITS-1:0] alu_op = INST_ALU_BITS'(execute_data.op_type);
    wire [INST_BR_BITS-1:0]   br_op = INST_BR_BITS'(execute_data.op_type);
    wire                   is_br_op = (execute_data.op_args.alu.xtype == ALU_TYPE_BRANCH);
    wire                  is_sub_op = inst_alu_is_sub(alu_op);
    wire                  is_signed = inst_alu_signed(alu_op);
    wire [1:0]             op_class = is_br_op ? inst_br_class(alu_op) : inst_alu_class(alu_op);

    wire [NUM_LANES-1:0][`XLEN-1:0] alu_in1 = execute_data.rs1_data;
    wire [NUM_LANES-1:0][`XLEN-1:0] alu_in2 = execute_data.rs2_data;

    wire [NUM_LANES-1:0][`XLEN-1:0] alu_in1_PC  = execute_data.op_args.alu.use_PC ? {NUM_LANES{to_fullPC(execute_data.PC)}} : alu_in1;
    wire [NUM_LANES-1:0][`XLEN-1:0] alu_in2_imm = execute_data.op_args.alu.use_imm ? {NUM_LANES{`SEXT(`XLEN, execute_data.op_args.alu.imm)}} : alu_in2;
    wire [NUM_LANES-1:0][`XLEN-1:0] alu_in2_br  = (execute_data.op_args.alu.use_imm && ~is_br_op) ? {NUM_LANES{`SEXT(`XLEN, execute_data.op_args.alu.imm)}} : alu_in2;
    ...
    for (genvar i = 0; i < NUM_LANES; ++i) begin : g_vote_calc
        ...
        assign vote_true[i]  = execute_data.tmask[i] && pred;
        assign vote_false[i] = execute_data.tmask[i] && ~pred;
    end
    ...
    if (NUM_LANES > 1) begin : g_shfl
        for (genvar i = 0; i < NUM_LANES; ++i) begin : g_i
            ...
            assign shfl_result[i] = execute_data.tmask[lane] ? alu_in1[lane] : alu_in1[i];
        end
    ...
    for (genvar i = 0; i < NUM_LANES; ++i) begin : g_alu_result
        ...
        always @(*) begin
            if (execute_data.op_args.alu.xtype == ALU_TYPE_OTHER) begin
                case (alu_op[2])
    ...
    VX_elastic_buffer #(
        ...
        .ready_in (execute_ready),
        .data_in  ({execute_data.uuid, execute_data.wid, execute_data.tmask, execute_data.rd, execute_data.wb, execute_data.pid, execute_data.sop, execute_data.eop, alu_result,   execute_data.PC, cbr_dest,   is_br_op,   br_op,   last_tid}),
        .data_out ({result_data.uuid,  result_data.wid,  result_data.tmask,  result_data.rd,  result_data.wb,  result_data.pid,  result_data.sop,  result_data.eop,  alu_result_r, PC_r,               cbr_dest_r, is_br_op_r, br_op_r, last_tid_r}),
    ...
    wire br_enable = result_fire && is_br_op_r && result_data.eop;
    wire br_taken = ((is_br_less ? is_less : is_equal) ^ is_br_neg) | is_br_static;
    wire [PC_BITS-1:0] br_dest = is_br_static ? from_fullPC(br_result) : cbr_dest_r;
    wire [NW_WIDTH-1:0] br_wid;
    `ASSIGN_BLOCKED_WID (br_wid, result_data.wid, BLOCK_IDX, `NUM_ALU_BLOCKS)
    ...
    for (genvar i = 0; i < NUM_LANES; ++i) begin : g_result
        ...
        assign result_data.data[i] = (is_br_op_r && is_br_static) ? PC_next : alu_result_r[i];
    end

    assign result_data.PC = PC_r;

`ifdef DBG_TRACE_PIPELINE
    always @(posedge clk) begin
        if (br_enable) begin
            `TRACE(2, ("%t: %s branch: wid=%0d, PC=0x%0h, taken=%b, dest=0x%0h (#%0d)\n",
                $time, INSTANCE_ID, br_wid, to_fullPC(result_data.PC), br_taken, to_fullPC(br_dest), result_data.uuid))
        end
    end
`endif
